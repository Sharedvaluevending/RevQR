...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['header_image'])) {
    ...
    if (!in_array($file['type'], $allowed_types)) {
        ...
        $upload_dir = __DIR__ . '/../public/templates/headers/';
        ...
        if (move_uploaded_file($file['tmp_name'], $target_path)) {
            ...
            $stmt = $pdo->prepare("
                INSERT INTO header_templates (
                    name, file_path, created_by, created_at
                ) VALUES (?, ?, ?, CURRENT_TIMESTAMP)
            ");
            
            if ($stmt->execute([$_POST['template_name'], 'templates/headers/' . $filename, $_SESSION['user_id']])) {
