...
--
-- Temporary view structure for view `campaign_items_view`
--

DROP TABLE IF EXISTS `campaign_items_view`;
/*!50001 DROP VIEW IF EXISTS `campaign_items_view`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `campaign_items_view` AS SELECT 
 1 AS `campaign_id`,
 1 AS `item_id`,
 1 AS `item_name`,
 1 AS `item_type`,
...
--
-- Temporary view structure for view `campaign_view`
--
...
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `campaign_view` AS SELECT 
 1 AS `campaign_id`,
 1 AS `business_id`,
 1 AS `campaign_name`,
 1 AS `campaign_description`,
 1 AS `campaign_type`,
 1 AS `is_active`,
 1 AS `tooltip`,
 1 AS `campaign_created_at`,
 1 AS `campaign_updated_at`*/;
SET character_set_client = @saved_cs_client;
...
CREATE TABLE `items` (
  ...
  `machine_id` int NOT NULL,
  `name` varchar(255) COLLATE utf8mb4_unicode_ci NOT NULL,
  `type` enum('snack','drink','pizza','side','other') COLLATE utf8mb4_unicode_ci NOT NULL,
  `description` text COLLATE utf8mb4_unicode_ci,
  `price` decimal(10,2) DEFAULT NULL,
  `list_type` enum('regular','in','vote_in','vote_out','showcase') COLLATE utf8mb4_unicode_ci NOT NULL,
...
CREATE TABLE `machines` (
  ...
  `slug` varchar(100) COLLATE utf8mb4_unicode_ci NOT NULL,
  `description` text COLLATE utf8mb4_unicode_ci,
  `type` enum('vote','promo') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'vote',
...
CREATE TABLE `qr_codes` (
  ...
  `campaign_type` enum('static','dynamic','cross_promo','stackable') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'static',
...
CREATE TABLE `votes` (
  ...
  KEY `idx_machine_vote_type` (`machine_id`,`vote_type`),
...
CREATE TABLE `winners` (
  ...
  KEY `idx_machine_week` (`machine_id`,`week_start`),
  KEY `fk_winners_item` (`item_id`),
  CONSTRAINT `fk_winners_item` FOREIGN KEY (`item_id`) REFERENCES `items` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
...
--
-- Final view structure for view `campaign_items_view`
--
...
/*!50001 VIEW `campaign_items_view` AS select `m`.`id` AS `campaign_id`,`i`.`id` AS `item_id`,`i`.`name` AS `item_name`,`i`.`type` AS `item_type`,`i`.`price` AS `price`,`i`.`list_type` AS `list_type`,`i`.`status` AS `status` from (`machines` `m` join `items` `i` on((`i`.`machine_id` = `m`.`id`))) where (`m`.`type` in ('vote','promo')) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `campaign_view` AS select `m`.`id` AS `campaign_id`,`m`.`business_id` AS `business_id`,`m`.`name` AS `campaign_name`,`m`.`description` AS `campaign_description`,`m`.`type` AS `campaign_type`,`m`.`is_active` AS `is_active`,`m`.`tooltip` AS `tooltip`,`m`.`created_at` AS `campaign_created_at`,`m`.`updated_at` AS `campaign_updated_at` from `machines` `m` where (`m`.`type` in ('vote','promo')) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2025-05-16 21:19:47
