...
echo "=== DEBUG AI ANALYTICS ===\n\n";

$business_id = 1;

// Test each query individually
echo "1. Testing casino participation query...\n";
$stmt = $pdo->prepare("SELECT * FROM business_casino_participation WHERE business_id = ?");
$stmt->execute([$business_id]);
$casino_participation = $stmt->fetch();
var_dump($casino_participation);

echo "\n2. Testing promotional ads query...\n";
$stmt = $pdo->prepare("
    SELECT 
        pa.feature_type,
...
var_dump($promotional_ads);

echo "\n3. Testing spin wheels query...\n";
$stmt = $pdo->prepare("
    SELECT 
        sw.*,
...
var_dump($spin_wheels);

echo "\n4. Testing pizza trackers query...\n";
$stmt = $pdo->prepare("
    SELECT 
        pt.*,
...
var_dump($pizza_trackers);

echo "\n5. Testing QR performance query...\n";
$stmt = $pdo->prepare("
    SELECT 
        qr.qr_type,
