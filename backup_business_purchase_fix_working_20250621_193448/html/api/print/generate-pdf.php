...
class LabelTemplate {
    ...
    public function __construct($config) {
        ...
        $this->page_width_mm = $config['page_width_mm'] ?? 215.9;
        $this->page_height_mm = $config['page_height_mm'] ?? 279.4;
...
// Enhanced Template configurations with precise measurements
$templates = [
    'avery_5160' => new LabelTemplate([
        ...
        'label_width_mm' => 66.675, // 2.625" exact
        'label_height_mm' => 25.4,   // 1.0" exact
        'cols' => 3,
        'rows' => 10,
    ...
    'avery_5658' => new LabelTemplate([
        ...
        'page_width_mm' => 215.9,
        'page_height_mm' => 279.4,
        'label_width_mm' => 50.8,    // 2.0" exact
        'label_height_mm' => 50.8,   // 2.0" exact
        'cols' => 2,
        'rows' => 5,
...
if (!isset($templates[$template]) && !$custom_template) {
    http_response_code(400);
...
try {
    ...
    $template_config = $custom_template ?? $templates[$template];
    
    // Apply fit-to-cut-out mode
    if ($fit_to_cut_out) {
        $template_config->fit_to_cut_out = true;
        $template_config->qr_fill_percentage = 100;
        $template_config->include_text = false;
    }
    ...
    class QRLabelPDF extends TCPDF {
        ...
        private function renderQRCode($qr, $x, $y, $size) {
            ...
            if ($qr_image_path) {
                // High-quality image rendering with 300 DPI
    ...
    // Generate PDF
    $pdf = new QRLabelPDF($template_config, $debug_mode);
    
    $labels_per_page = $template_config->getLabelsPerPage();
    ...
    $template_suffix = $custom_template ? 'custom' : $template;
    $filename = "qr_labels_{$template_suffix}_{$timestamp}.pdf";
    ...
    echo json_encode([
        ...
        'template_config' => [
            'label_size' => $template_config->label_width_mm . 'mm × ' . $template_config->label_height_mm . 'mm',
            'qr_fill_percentage' => $template_config->qr_fill_percentage . '%',
            'layout' => $template_config->cols . ' × ' . $template_config->rows,
            'page_size' => $template_config->page_width_mm . 'mm × ' . $template_config->page_height_mm . 'mm'
        ]
    ]);
