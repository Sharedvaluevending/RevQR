...
class NayaxDiscountManager {
    ...
    public function purchaseDiscountCode($user_id, $qr_store_item_id, $nayax_machine_id = null) {
        try {
            ...
            // Deduct QR coins from user using new spendCoins method
            ...
            if ($item['business_id']) {
                $business_share_coins = round($item['qr_coin_cost'] * (1 - $this->config['commission_rate']));
                ...
                $business_wallet->addCoins(
                    ...
                    [
                        ...
                        'original_price' => $item['qr_coin_cost'],
