...
// Set JSON content type and prevent caching
...
try {
    // Debug logging
    error_log("PURCHASE DEBUG: User ID: " . ($_SESSION['user_id'] ?? 'NOT SET'));
    error_log("PURCHASE DEBUG: Request method: " . $_SERVER['REQUEST_METHOD']);
    
    // Get JSON input
    $raw_input = file_get_contents('php://input');
    error_log("PURCHASE DEBUG: Raw input: " . $raw_input);
    
    $input = json_decode($raw_input, true);
    error_log("PURCHASE DEBUG: Parsed input: " . json_encode($input));
    
    if (!$input || !isset($input['item_id'])) {
        error_log("PURCHASE DEBUG: Invalid input - input: " . json_encode($input));
    ...
    // Deduct QR coins using the new spendCoins method
    $qr_transaction_result = QRCoinManager::spendCoins(
        $_SESSION['user_id'],
        $item['qr_coin_cost'],
    ...
    if (!$qr_transaction_result['success']) {
        $pdo->rollback();
        throw new Exception($qr_transaction_result['error'] ?? 'Failed to process QR coin transaction. Please try again.');
    }
    ...
    echo json_encode([
        ...
        'new_balance' => $qr_transaction_result['balance'],
        'amount_spent' => $qr_transaction_result['amount_spent']
    ]);
