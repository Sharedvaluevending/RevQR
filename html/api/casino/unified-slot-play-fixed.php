...
/**
 * FIXED UNIFIED SLOT MACHINE ENDPOINT
 * 
 * CRITICAL FIX: This endpoint now uses the SAME symbol loading logic as the frontend
 * to eliminate the frontend/backend symbol array mismatches that were causing 
 * visual wins to not match actual payouts.
 * 
 * Key Changes:
 * 1. Load user avatars the same way frontend does
 * 2. Use identical fallback logic for missing avatars  
 * 3. Ensure frontend and backend have identical symbol arrays
 * 4. Fix wild symbol positioning consistency
 */
...
try {
    ...
    // PHASE 8: GENERATE CRYPTOGRAPHIC SIGNATURE FOR RESULTS
    ...
    echo json_encode([
        ...
        'spin_info' => $updated_spin_info,
        'signature' => $result_signature,
        'symbols_used' => count($symbols),
        'debug' => [
            'symbols_loaded' => count($symbols),
            'frontend_backend_synced' => true
        ]
    ]);
    ...
    error_log("Slot machine error for user $user_id: " . $e->getMessage());
...
/**
 * Generate winning 3x3 slot grid using proper symbols
 */
...
/**
 * Generate losing 3x3 slot grid using proper symbols
 */
...
/**
 * Calculate secure payouts using proper symbol array
 */
function calculateSecurePayouts($results, $bet_amount) {
    ...
    foreach ($lines as $line) {
        ...
        if ($symbolsMatch($s1, $s2) && $symbolsMatch($s2, $s3) && $symbolsMatch($s1, $s3)) {
            ...
            // Calculate payout based on symbol level and rarity
            if ($wild_count === 3) {
                // Triple wild jackpot
                return [
...
function getWeightedRandomSymbol($symbols) {
    // Give higher weight to common symbols for balanced gameplay
    $weights = [];
    foreach ($symbols as $symbol) {
        $weight = 10; // Base weight
        if ($symbol['rarity'] === 'common') $weight = 40;
        elseif ($symbol['rarity'] === 'uncommon') $weight = 25;
        elseif ($symbol['rarity'] === 'rare') $weight = 15;
        elseif ($symbol['rarity'] === 'epic') $weight = 8;
        elseif ($symbol['rarity'] === 'legendary') $weight = 4;
        elseif ($symbol['rarity'] === 'mythical') $weight = 2;
        elseif ($symbol['isWild']) $weight = 5; // Wilds are special
        
        $weights[] = $weight;
    }
    
    return weightedRandomSelect($symbols, $weights);
}
...
function getRandomSymbolDifferentFrom($symbols, $exclude_symbols) {
    ...
    return !empty($available) ? $available[mt_rand(0, count($available) - 1)] : $symbols[0];
}
