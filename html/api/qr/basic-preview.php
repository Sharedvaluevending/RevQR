...
try {
    ...
    if (!empty($data['action']) && $data['action'] === 'regenerate' && !empty($data['qr_id'])) {
        $qr_id = intval($data['qr_id']);
        
        // Get QR code details from database
        $stmt = $pdo->prepare("
            SELECT id, code, qr_type, url, 
        ...
        $content = $qr_record['content'] ?: $qr_record['url'] ?: 'https://example.com';
        
        // Build QR options for regeneration
        $options = [
