-- Casino Management Tables
-- Creates tables for global casino settings and prize templates
...
-- Business Prize Configuration Table (links businesses to prize templates)
CREATE TABLE IF NOT EXISTS casino_business_prizes (
    id INT AUTO_INCREMENT PRIMARY KEY,
    business_id INT NOT NULL,
    prize_template_id INT NOT NULL,
    custom_probability DECIMAL(5,2) NULL COMMENT 'Business-specific probability override',
    custom_multiplier_min INT NULL COMMENT 'Business-specific min multiplier override',
    custom_multiplier_max INT NULL COMMENT 'Business-specific max multiplier override',
    is_active TINYINT(1) DEFAULT 1,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    
    FOREIGN KEY (business_id) REFERENCES businesses(id) ON DELETE CASCADE,
    FOREIGN KEY (prize_template_id) REFERENCES casino_prize_templates(id) ON DELETE CASCADE,
    UNIQUE KEY unique_business_prize (business_id, prize_template_id),
    INDEX idx_business (business_id),
    INDEX idx_active (is_active)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- Casino Analytics Summary Table (for faster reporting)
CREATE TABLE IF NOT EXISTS casino_analytics_summary (
    ...
    FOREIGN KEY (business_id) REFERENCES businesses(id) ON DELETE CASCADE,
    UNIQUE KEY unique_business_period (business_id, date_period, period_type),
...
INSERT INTO casino_prize_templates (prize_name, prize_type, prize_value, win_probability, multiplier_min, multiplier_max, is_jackpot) VALUES
